<template>
  ...
  >
    <span class="text-caption"> Addresses [{{ nameSearch.length }}] </span>
    ...
    <q-list>
      <q-item
        ...
        :key="result.id"
        @click.prevent="
          router.push({ name: 'address', params: { id: result.id } })
        "
      >
...
import { addressesMap } from '@koiner/contracts/components/contract';

export default defineComponent({
  name: 'AddressesNameSearch',
  props: {
    search: {
      required: true,
      type: String,
  ...
  setup(props) {
    ...
    const nameSearch = computed(() => {
      return Object.entries(addressesMap)
        .filter(([_, name]) =>
